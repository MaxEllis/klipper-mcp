...
def record_slice(gcode_filename: str, model_name: str, geometry_hash: str, settings: dict,
    ...
    with connect(create=True) as conn:
        cur = conn.execute(
            ...
            " VALUES (?,?,?,?,?,?)",
            (printer_id, sliced_at or time.time(), model_name, geometry_hash, gcode_filename,
...
def record_outcome(job: dict, printer_id: str = "swx2") -> int:
    ...
    with connect(create=True) as conn:
        ...
        row = conn.execute(
            ...
            " ORDER BY sliced_at DESC LIMIT 1", (fn,)).fetchone()
        if row:
    ...
    with connect(create=True) as conn:
        if gcode_filename:
            row = conn.execute("SELECT id FROM prints WHERE gcode_filename=? AND printed_at IS NOT NULL"
                               " ORDER BY printed_at DESC LIMIT 1", (gcode_filename,)).fetchone()
        else:
            row = conn.execute("SELECT id FROM prints WHERE printed_at IS NOT NULL"
                               " ORDER BY printed_at DESC LIMIT 1").fetchone()
        if not row:
...
def recall(model_name: str | None = None, geometry_hash: str | None = None, limit: int = 10) -> list[dict]:
    ...
    with connect() as conn:
        rows = conn.execute(f"SELECT * FROM prints{clause} ORDER BY COALESCE(printed_at, sliced_at) DESC LIMIT ?",
                            (*args, limit)).fetchall()
...
def _row(r: sqlite3.Row) -> dict:
    d = dict(r)
    settings = json.loads(d.pop("settings_json") or "null")
    d["settings_summary"] = settings_summary(settings) if settings else {}
    return d
